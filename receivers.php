...
class R_List_Table extends WP_List_Table {
	 ...
	 */
	 function __construct() {
		...
		'singular'=> 'receiver', //Singular label
		'plural' => 'receivers', //plural label, also this well be one of the table css class
...
function column_default( $item, $column_name ) {
  ...
  $actions = array(
            'edit'      => sprintf('<a href="?page=%s&action=%s&id=%s">Edit</a>','add_receiver','edit',$item['id']),
...
function extra_tablenav( $which ) {
	if ( $which == "top" ){
		...
		echo"<div id='icon-users' class='icon32'></div><h2>List of People  Helped.</h2>";
	}
	if ( $which == "bottom" ){
